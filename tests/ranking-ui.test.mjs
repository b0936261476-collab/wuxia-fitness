...
import { newState, playerLevelSum, updateRanking, addExp } from "../src/engine/game.js";
...
test("nextIntegerMilestone:第 9000 名的下一道坎是 8500", () => {
  assert.equal(nextIntegerMilestone(9000), 8500);
});

test("nextIntegerMilestone:剛好站在坎上,下一道是再前面五百名", () => {
  assert.equal(nextIntegerMilestone(8500), 8000);
});

test("nextIntegerMilestone:進了第一道坎以內就不再報關口", () => {
  assert.equal(nextIntegerMilestone(500), null);
  assert.equal(nextIntegerMilestone(120), null);
});

test("nextIntegerMilestone 與 integerMilestonesCrossed 對得上:報的坎跨過去就會被播報", () => {
  const m = nextIntegerMilestone(9000);
  assert.deepEqual(integerMilestonesCrossed(9000, m), [m]);
});

// ---------- 名次與稱號(§9.7.1 / §8.1) ----------

test("新角色從萬人總冊末段起算(§9.7.1 #9,000 開外)", () => {
  const state = newState();
  const snap = playerRankSnapshot(playerLevelSum(state), data.npcs);
  assert.ok(snap.rank >= 9000, `rank=${snap.rank}`);
});
...
test("updateRanking:超越具名百強 → flag 落地、俠名入帳、只算一次", () => {
  const state = newState();
  updateRanking(state, data); // 首次呼叫對時,不視為跨越
  const gains = {};
  for (const d of DIMENSIONS) gains[d] = thresholdForLevel(30);
  addExp(state, gains, data);

  const result = updateRanking(state, data);
  ...
  const again = updateRanking(state, data);
